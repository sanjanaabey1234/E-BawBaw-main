...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $sql = "INSERT INTO Users (username, user_password, user_email, type, date_created) VALUES (?, ?, ?, ?, NOW())";
    $stmt = $conn->prepare($sql);
    if (!$stmt) {
        die("Prepare failed: (" . $conn->errno . ") " . $conn->error);
    }
    $stmt->bind_param("ssss", $Uusername, $password, $email, $user_type);
    if (!$stmt->execute()) {
        die("Execute failed: (" . $stmt->errno . ") " . $stmt->error);
    }

    // Success
    echo "<script>alert('User added successfully.'); window.location.href = 'add_new_user.php';</script>";
...
<head>
    ...
    <title>E - BawBaw | Add New User</title>
    <!-- Custom Styling -->
                        ...
                        <div class="card card-default">
                            ...
                            <div class="card-body">
                                ...
                                <div class="card-footer">
                                    <button type="submit" class="btn btn-primary" style="width: 100%;">Add New
                                        User</button>
                                </div>
                            </div>
                    </form>
